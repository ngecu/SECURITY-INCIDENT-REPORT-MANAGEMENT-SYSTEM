...
const handleSearch = (value) => {
  ...
  const filteredLeads = leads.filter((lead) => {
    ...
    return (
      lead.status[lead.status.length - 1].type === 'Enquiry' && // Check if lead status type is 'Enquiry'
...
const handleStatusChange = (value) => {
  const filteredLeads = leads.filter((lead) => {
    return lead.status[lead.status.length - 1].type === 'Enquiry' && value === 'Enquiry';
  });
  setFilteredLeads(filteredLeads);
};
...
const handleDateRangeChange = (dates, dateStrings) => {
  ...
  useEffect(() => {
    if (leads && leads.length > 0) {
      // Filter leads to include only 'Enquiry' type leads
      const filteredLeads = leads.filter((lead) => {
        return lead.status[lead.status.length - 1].type === 'Enquiry';
      });
      setFilteredLeads(filteredLeads);
    }
  }, [leads]);
  ...
  <Loader />
          ...
          <h1>My Enquiry Leads({filteredLeads && filteredLeads.length})</h1>
